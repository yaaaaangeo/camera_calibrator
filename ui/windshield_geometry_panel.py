"""
camera_calibrator.ui.windshield_geometry_panel
==============================================================

Priority 7 안정화 - `ui/windshield_workspace.py`(God Object)에서 분리한
Windshield Geometry(③ Windshield Model 탭) 전용 UI. `GeometryPanelMixin`은
독립적으로 쓰이지 않고 항상 `WindshieldWorkspace`(ui/windshield_workspace.py)
에 다른 패널 mixin들과 함께 다중 상속되어, 하나의 `self`(같은 QWidget
인스턴스)를 공유한다 - 이 파일을 포함한 모든 패널 파일이 서로 다른 파일에
있어도 `self._windshield_results`처럼 원래부터 있던 공유 상태/다른 패널의
메서드 호출이 전혀 변경 없이 그대로 동작한다(Python MRO가 어느 mixin에
정의됐든 `self.method_name()`을 찾아준다).

이 파일은 로직을 하나도 새로 추가/변경하지 않았다 - `ui/windshield_workspace.py`
에 있던 메서드를 그대로 옮겼을 뿐이다(Geometry UI만 담당 - 계산 로직은
여전히 calibration/windshield/*에 있다).
"""
...
class GeometryPanelMixin:
    """③ Windshield Model 탭(Geometry 계산/결과 표시) 전용 UI. 계산
    로직은 전혀 갖지 않는다 - 입력을 모아 `WindshieldCalibrationWorker`를
    실행하고, 결과(`WindshieldCalibrationResult`)를 받아 그리기만 한다."""
