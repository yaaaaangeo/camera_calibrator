...
def _wait_with_heartbeat(future, progress_signal, label: str, cancel_check=None):
    """future.result()를 기다리는 동안 주기적으로 progress_signal에 경과
    시간을 emit한다. 계산 자체는 자식 프로세스에서 계속 진행 중이다.

    cancel_check가 주어지고 True를 돌려주면 PipelineCancelled를 던진다.
    취소 버튼이 이미 이 future를 만든 executor를 강제 종료했을 수도 있는데,
    그 경우 future.result()가 BrokenProcessPool로 먼저 반응한다 - 취소
    요청이 있었다면 그것도 정상 취소로 취급한다.
    """
    ...
    while True:
        try:
            ...
            if cancel_check is not None and cancel_check():
                raise PipelineCancelled("계산이 취소되었습니다.")
            elapsed = time.monotonic() - start
            progress_signal.emit(f"{label} ({elapsed:.0f}초 경과 - 계산 중입니다)")
        except BrokenProcessPool:
            if cancel_check is not None and cancel_check():
                raise PipelineCancelled("계산이 취소되었습니다.") from None
            raise
...
class PipelineWorker(QObject):
    ...
    def __init__(
        ...
        self.calibration_method = (
            ...
            else CalibrationMethod(str(calibration_method))
        )
        self.dataset: Dataset | None = None
        self._cancel_requested = False
        self._current_executor = None  # 취소 버튼이 강제 종료할 수 있게 붙잡아두는 참조
    ...
    def _register_executor(self, executor) -> None:
        ...
        if self._cancel_requested:
            # request_cancel()이 executor가 만들어지기 직전에 호출된 아주
            # 좁은 틈이면 위 호출에서는 아무것도 못 죽였을 수 있다 - 여기서
            # 한 번 더 확인해서 놓치지 않게 한다.
            terminate_executor_processes(executor)
    ...
    def run(self) -> None:
        try:
            ...
            self.progress.emit("Ideal Pinhole / Brown-Conrady / Rational / Fisheye 모델 계산 중...")
            ...
            with ProcessPoolExecutor(max_workers=1) as executor:
                ...
                if self.calibration_method == CalibrationMethod.OBJECT_RELEASING:
                    ro_result = object_releasing_result
                    if ro_result is None or not ro_result.success:
                        reason = ro_result.error_message if ro_result is not None else "no Object-Releasing result"
                        raise RuntimeError(f"Object-Releasing calibration failed: {reason}")
            self._current_executor = None
            ...
            self.progress.emit("사용자 요청으로 계산을 취소했습니다.")
            self.cancelled.emit()
...
class BenchmarkDetectionWorker(QObject):
    ...
    (PipelineWorker)은 이미 QThread로 분리돼 있었지만 이 경로만 빠져 있었다.
    """
