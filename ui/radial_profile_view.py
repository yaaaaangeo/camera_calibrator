...
백엔드(calibration/radial_profile.py) 결과를 그리기만 한다 - dataset_view.py와
동일한 원칙(계산은 backend, UI는 시각화만).

외부 차트 라이브러리(matplotlib, QtCharts) 의존성을 추가하지 않기 위해
QPainter로 직접 막대그래프를 그린다 (dataset_view.py의 CoverageGridWidget과
같은 접근 방식).
"""
...
def _lerp_color(t: float) -> QColor:
    """0(낮음, 초록) ~ 1(높음, 빨강) 사이 선형 보간. dataset_view.py의
