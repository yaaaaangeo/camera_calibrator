...
class WindshieldWorkspace(QWidget):
    ...
    def _build_model_tab(self) -> QWidget:
        ...
        self.stats_table = _ScrollTable(len(_STATS_ROWS) + len(_REGIONAL_ROWS) + 3, 2)
    ...
    def _on_run_windshield_calibration(self) -> None:
        ...
        self._windshield_config.windshield_model = self._selected_windshield_model()
    ...
    def _display_result(self, result: WindshieldCalibrationResult) -> None:
        ...
        self.export_button.setEnabled(result.windshield_model == WindshieldModelType.BASELINE)
        ...
        for region in _REGIONAL_ROWS:
            v = getattr(result.regional_error, region) if result.regional_error else None
            self.stats_table.setItem(row, 0, QTableWidgetItem(_fmt(v)))
            self.stats_table.setItem(row, 1, QTableWidgetItem("N/A"))
            row += 1
        edge_avg = regional_edge_average(result.regional_error) if result.regional_error else None
        self.stats_table.setItem(row, 0, QTableWidgetItem(_fmt(edge_avg)))
        self.stats_table.setItem(row, 1, QTableWidgetItem("N/A"))
    ...
    def _on_export_windshield_yaml(self) -> None:
        result = self._windshield_results.get(WindshieldModelType.BASELINE)
        if result is None or not result.success:
            QMessageBox.warning(self, "Export", "Export할 Baseline 결과가 없습니다.")
            return
    ...
    def _build_comparison_tab(self) -> QWidget:
        ...
        self.comparison_table = _ScrollTable(4, 0)
        self.comparison_table.setVerticalHeaderLabels(["Hold-out RMS", "P95", "Edge RMS", "Improvement %"])
    ...
    def _refresh_comparison_table(self) -> None:
        ...
        for col, model in enumerate(present):
            ...
            edge = regional_edge_average(result.regional_error) if result.regional_error else None
            ...
            self.comparison_table.setItem(
                3, col, QTableWidgetItem(f"{improvement:.0f}%" if improvement is not None else "-")
            )
