...
def export_opencv_yaml(
    ...
    fs.write("camera_model", result.model_name.value)
    fs.write(
        "calibration_method",
        result.calibration_method.value
        if hasattr(result.calibration_method, "value") else str(result.calibration_method),
    )
    fs.write(
        "target_type",
        pattern_config.type.value if hasattr(pattern_config.type, "value") else str(pattern_config.type),
    )
