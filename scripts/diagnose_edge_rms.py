...
사용법 (레포 루트에서 실행):
    python scripts/diagnose_edge_rms.py <이미지 폴더> --squares-x 11 --squares-y 8 \
...
# 이 스크립트는 camera_calibrator/scripts/ 아래에 있으므로, calibration
# 패키지를 찾으려면 부모(레포 루트) 디렉터리를 sys.path에 넣어야 한다.
# 레포 루트가 아닌 다른 폴더에서 실행해도 항상 이 파일 기준 상대 경로로
# 계산하므로 동작한다.
...
try:
    ...
    print(
        ...
        "레포 루트(camera_calibrator/)에서 python scripts/diagnose_edge_rms.py로 실행하세요."
    )
    sys.exit(1)
