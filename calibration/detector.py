...
def build_circle_grid_object_points(pattern: PatternConfig) -> np.ndarray:
    """PatternConfig -> circle grid center object points, shape (N,1,3)."""
    cols = pattern.squares_x
    rows = pattern.squares_y
    spacing = float(pattern.square_size)
    objp = np.zeros((rows * cols, 3), dtype=np.float32)
    for row in range(rows):
        for col in range(cols):
            idx = row * cols + col
            if pattern.circle_grid_type == CircleGridType.ASYMMETRIC:
                objp[idx, 0] = (2 * col + (row % 2)) * spacing
                objp[idx, 1] = row * spacing
            else:
                objp[idx, 0] = col * spacing
                objp[idx, 1] = row * spacing
    return objp.reshape(-1, 1, 3)
...
def detect_circle_grid(
    ...
    if not found or centers is None:
        return DetectionResult(
            ...
            num_corners=0,
            failure_reason=(
                f"Circle grid centers not found ({grid_label}, {pattern.squares_x}x{pattern.squares_y}). "
                "Check rows/columns, spacing, contrast, and whether the full grid is visible."
            ),
        )

    centers = centers.astype(np.float32).reshape(-1, 1, 2)
    object_points = build_circle_grid_object_points(pattern)
    if centers.shape[0] != object_points.shape[0]:
        return DetectionResult(
            image_id=image_id,
            success=False,
            num_corners=int(centers.shape[0]),
            failure_reason=(
                f"Circle grid point count mismatch: detected {centers.shape[0]}, "
                f"expected {object_points.shape[0]}."
            ),
        )
...
def detect_aprilgrid(
    ...
    if pattern.aprilgrid_variant == AprilGridVariant.KALIBR:
        logger.info(
            "Kalibr AprilGrid variant selected; using OpenCV AprilTag detector path. "
            "Kalibr border compatibility depends on the generated target."
        )
...
def build_detect_fn(pattern: PatternConfig) -> Callable[[np.ndarray, str], DetectionResult]:
    ...
    """
    pattern_type = pattern.type if isinstance(pattern.type, PatternType) else PatternType(str(pattern.type))
    if pattern_type == PatternType.CHARUCO:
    ...
    supported = (PatternType.CHARUCO, PatternType.CHESSBOARD, PatternType.CIRCLE_GRID, PatternType.APRILGRID)
    raise ValueError(
        f"현재 지원하는 패턴 타입은 {', '.join(p.value for p in supported)}뿐입니다 "
        f"(입력: {pattern_type.value}). 검출 로직이 아직 구현되지 않았습니다."
    )
...
def maximum_pattern_corners(pattern: PatternConfig) -> int:
    ...
    """
    pattern_type = pattern.type if isinstance(pattern.type, PatternType) else PatternType(str(pattern.type))
    if pattern_type in (PatternType.CHARUCO, PatternType.CHESSBOARD):
        return max(0, (pattern.squares_x - 1) * (pattern.squares_y - 1))
    if pattern_type == PatternType.CIRCLE_GRID:
        return max(0, pattern.squares_x * pattern.squares_y)
    if pattern_type == PatternType.APRILGRID:
...
def detect_dataset(
    ...
    *,
    ...
    on_executor_ready: Optional[Callable[[object], None]] = None,
) -> Dataset:
    ...
    Args:
        ...
        progress_callback: 이미지 한 장의 검출이 끝날 때마다 ``(완료 수, 전체 수)``를
            ...
            도달한다.
        cancel_check: 호출될 때마다 취소 여부(bool)를 돌려준다. True가 되면
            남은 이미지를 처리하지 않고 PipelineCancelled를 던진다 - 병렬
            경로에서는 이미 실행 중인 워커 프로세스도 즉시 강제 종료한다.
        on_executor_ready: 병렬 경로에서만 쓰인다. 내부에서 만든
            ProcessPoolExecutor를 인자로 즉시 넘겨줘서, 호출부(취소 버튼을
            누를 수 있는 GUI 스레드)가 나중에 그 executor를 붙잡고 강제
            종료할 수 있게 한다.
    """
    ...
    if parallel and len(image_paths) > 1 and cpu_count > 1:
        ...
        if progress_callback is None and cancel_check is None and on_executor_ready is None:
            ...
            pairs = _detect_dataset_parallel(
                image_paths, pattern, effective_workers,
...
def _detect_dataset_parallel(
    ...
    with ProcessPoolExecutor(
        ...
        if on_executor_ready is not None:
            ...
            # 있게 만들자마자 바로 넘긴다 (취소 시 terminate_executor_processes로
            # 강제 종료하기 위함).
        ...
        try:
            ...
            # 취소 버튼이 GUI 스레드에서 이 executor를 이미 강제 종료했을 때도
            # future.result()가 이 예외로 나타난다 - 취소 요청이 있었다면 정상
            # 취소로 취급하고, 없었다면(진짜 워커 프로세스 이상) 그대로 올린다.
            if cancel_check is not None and cancel_check():
                raise PipelineCancelled("코너 검출이 취소되었습니다.") from None
            raise
        return results
