...
(검출/품질/모델 계산/검증/export) 전부는 이미지가 폴더에서 왔는지 bag에서
...
class TopicInfo:
    ...
    BagImageTopic은 그대로 두고, _list_topics_by_type 내부에서는 이 타입을 쓴다.
    """
    name: str
    msg_type: str
    count: int | None = None
...
def read_bag_duration(bag_path: str) -> float:
    """bag 전체 duration(초). Camera Topic 선택 UI의 timeline scrubbing에 쓰인다."""
    _require_rosbags()
    with _open_reader(bag_path) as reader:
