...
ui/worker.py의 PipelineWorker/OutlierPruneWorker가 "GUI 스레드가 아니라
완전히 별도의 OS 프로세스"에서 무거운 계산(3모델 캘리브레이션 + Hold-out
검증, 이상치 반복 재계산)을 돌리기 위한 진입점 함수들을 모아둔다.
...
def run_models_and_validation(
    ...
    return calibration_results, validation_results
...
def run_outlier_pruning_and_validation(
    ...
    명시적으로 반환하고, 호출부(OutlierPruneWorker)가 자기 쪽 dataset 참조를
    통째로 이걸로 교체해야 한다 - 안 그러면 "이상치 제외"가 화면에 반영되지
    않는 조용한 버그가 생긴다.
    """
