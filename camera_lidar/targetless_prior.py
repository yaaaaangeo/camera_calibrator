...
def load_direct_visual_calib(path: "str | Path", source: str = "auto") -> TargetlessPrior:
    ...
    source: "auto" (prefer T_lidar_camera, then init_T_lidar_camera_auto,
    then init_T_lidar_camera), or one of "final" / "auto_initial" /
    "manual_initial" to force a specific key. Raises ValueError with a
    specific message on any malformed/missing/non-finite input."""
    ...
    if source == "auto":
        actual_key = next((k for k in _AUTO_SELECT_ORDER if k in results), None)
        if actual_key is None:
            raise ValueError(
                f"calib.json['results'] has none of the expected keys {_AUTO_SELECT_ORDER}"
            )
    else:
