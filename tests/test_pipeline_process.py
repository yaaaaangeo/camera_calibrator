...
실제 사용자 버그: PipelineWorker/OutlierPruneWorker(ui/worker.py)가 무거운
계산(3모델 캘리브레이션 + Hold-out 검증)을 QThread 안에서 직접 돌려서,
...
from calibration.types import CameraModelType, Dataset, FrameStatus
...
def test_run_models_and_validation_survives_a_real_process_roundtrip(
    ...
    with ProcessPoolExecutor(max_workers=1) as executor:
        ...
        calibration_results, validation_results = future.result(timeout=120)

    assert set(calibration_results) == {
        CameraModelType.PINHOLE, CameraModelType.EXTENDED_PINHOLE, CameraModelType.FISHEYE,
    }
